...
class Submission(val controller: ActorRef, workflow: Conv.MessageRoute, endpoints: Conv.MessageRoute*)(implicit val group: String, val router: Router) extends Workflow {
  ...
  val error_channel = s"direct:$group-error-channel"
  ...
  // Allows for the error handler to be overridden - e.g. in testing
  def error_handler {
    from(error_channel) {
      oneway >=> to("log:SubmissionErrorChannel?level=ERROR&showAll=true") >=> to(s"jms:queue:$group-error")
    }
  }
  ...
  from(msg_store) {
    ...
    attempt(retries) {
      ...
      choose {
        ...
        // For SQL errors: after 2 failures, delay original message and retry
        case (exn: SQLException, s: RetryState) if (retries >= s.count && s.count > retries - 2) => retry(s)
        case (exn: SQLException, s: RetryState) if (retries - 2 >= s.count && s.count > 0) => orig(s) >=> delay(10.seconds) >=> retry(s)
        // If all else fails, send original message to the error channel
        case (exn: Exception, s: RetryState) => orig(s) >=> to(error_channel) >=> failWith(exn)
    }
  }
}
